...
- **Blossom URL Utilities Consolidation**: Unified `extractBlossomHash` function into `src/lib/blossom-url.ts` as the single source of truth. Removed duplicate implementation from `video-event.ts`. The consolidated version includes non-Blossom server detection (video.nostr.build, cdn.nostrcheck.me), bare hash support, and lowercase normalization. Created comprehensive test suite with 22 tests covering all edge cases (blossom-url.test.ts)
- **Server Availability Utilities Extraction**: Extracted shared types (`ServerStatus`, `ServerInfo`, `ServerAvailability`) and functions (`extractServerList`, `checkBlossomServer`) from `useVideoServerAvailability.ts` and `useMultiVideoServerAvailability.ts` into new shared module `src/lib/server-availability.ts`. Reduces code duplication and improves maintainability. Hooks now re-export types for backward compatibility
