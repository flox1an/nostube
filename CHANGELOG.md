...
- **Upload Draft Title Not Persisting on Navigation**: Fixed draft title and form field changes not appearing in draft picker after navigation. Root causes: (1) React state updates are async - useEffect might not run before navigation, (2) React state could be out of sync with localStorage. Solutions: (1) Refactored `useUploadDrafts` to use localStorage as single source of truth with version counter for re-renders, (2) Added explicit save in back button handler that synchronously captures current form state before navigating. All form fields (title, description, tags, language, etc.) now reliably persist immediately when clicking "Back to drafts" (useUploadDrafts.ts:55-246, VideoUpload.tsx:91-128,247)
