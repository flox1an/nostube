...
- **Video Player Resume Position Not Working**: Fixed resume from last play position (stored in localStorage) and `?t=` URL parameter not seeking to the correct time. Two issues: (1) Initial position was set before video metadata loaded - now waits for `loadedmetadata` event. (2) The `loadedmetadata` listener captured a stale `initialPlayPos` value (0) due to closure - now uses a ref to get latest value. (3) Position wasn't restored if `videoDuration` was undefined - now restores position even without duration info (VideoPlayer.tsx, useVideoPlayPosition.ts)
