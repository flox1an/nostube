...
- **Blurhash Thumbnail Placeholders**: Video thumbnails now show a blurred low-quality image placeholder (LQIP) while the full thumbnail loads. Uses the blurhash from video imeta tags to generate a 32x32 PNG data URL that displays instantly, providing a smooth loading experience instead of a gray skeleton. Falls back to skeleton placeholder when no blurhash is available (VideoCard.tsx, VideoSuggestions.tsx, blurhashDataURL.ts)
