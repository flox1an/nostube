...
- **Tauri Desktop App**: Native desktop application using Tauri v2, wrapping the web app with system webview for macOS/Windows/Linux (~9MB bundle size vs 150MB+ Electron), with native window fullscreen support for video player and NosTube app icon
...
- **Tauri Fullscreen**: Added missing window fullscreen permissions (`core:window:allow-set-fullscreen`, `core:window:allow-is-fullscreen`) to Tauri capabilities
