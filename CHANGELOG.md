...
- Routine dependency audit: `npm audit` reports 0 vulnerabilities (no CVEs found); bumped `@cashu/cashu-ts`, `@types/react-dom`, `eslint`, `hono`, `i18next`, `lint-staged`, `lucide-react`, `mediabunny`, `nostr-tools`, and `react-resizable-panels` to their latest in-range patch/minor versions; `@vitejs/plugin-react` (6.x) and `typescript` (7.x) major upgrades deliberately deferred
- `DraftPicker`'s delete-draft test now matches the delete button's icon via `svg.lucide-trash` instead of `svg.lucide-trash-2` — `lucide-react` 1.41.0 aliases the `Trash2` icon component to the same underlying `trash` icon as `Trash`, so its auto-generated class name changed from `lucide-trash-2` to `lucide-trash`
