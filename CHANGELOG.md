...
- **Notifications Infinite Loading Spinner**: Fixed notification bell showing infinite loading spinner. Added defensive error handling around relay subscription setup to ensure the Promise always resolves. The overall timeout is now set up before the subscription to guarantee it fires even if subscription creation fails. Also properly sets `isLoading` to `false` when returning early due to empty relay list, and increased relay query timeout from 5s to 15s for better reliability with slow relays (useNotifications.ts:84-91,114-184)
