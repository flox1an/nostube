...
- **Upload Draft Title Not Persisting on Navigation**: Fixed draft title changes being lost when navigating away quickly (within 3 seconds). Draft saves now immediately persist to localStorage while only debouncing the Nostr sync (kind 30078) to minimize network calls. Added flush mechanism to ensure pending Nostr syncs complete on component unmount. Includes test coverage for immediate localStorage saves (useUploadDrafts.ts:163-181)
