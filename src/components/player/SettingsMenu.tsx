...
function SubmenuHeader({ title, onBack }: SubmenuHeaderProps) {
  return (
    <button
      type="button"
      className="flex items-center gap-2 w-full px-4 py-2.5 text-white text-sm border-b border-white/10 hover:bg-white/10 transition-colors"
      onClick={onBack}
    >
...
function SelectableItem({ label, isSelected, onClick }: SelectableItemProps) {
  return (
    <button
      type="button"
      className={`flex items-center justify-between w-full px-4 py-2.5 text-white text-sm transition-colors ${
