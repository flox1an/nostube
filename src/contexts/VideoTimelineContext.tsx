...
export function VideoTimelineProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadTimeline = useCallback((type: VideoType, authors?: string[]) => {
    const seq = ++logSeq;
    ...
    // Create filters
    const filter:Filter = authors ? { kinds: getKindsForType(type), authors, limit: 50 } : { kinds: getKindsForType(type), limit: 50 };
    ...
    const videos$ = eventStore.timeline(filter).pipe(map(events => processEvents(events, readRelays, blockedPubkeys)));
      ...
      createTimelineLoader(pool, readRelays, filter, { limit: 50 })()
        .pipe(
          finalize(() => {
            ...
            console.log(`[${finalizeSeq}] loader finalize called, setting timestamp for hash:`, hash);
